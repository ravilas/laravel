(function ($, window, document, MediaElement) {
    ...
    function dispatch() {
        /*var info = {ids:[],classes:[],dataids:[]}
        $('*').each(function(index, el) {
            if(el.id) info.ids.push('#'+el.id);
            if($(el).data( "id" )) info.dataids.push('data-id="'+$(el).data( "id" )+'"');
            if($(el).attr('class')) {
                var allClasses = $(el).attr('class').split(' ');
                for (var i =0, t = allClasses.length - 1; i< t; i++) {
                    info.classes.push('.'+allClasses[i]);
                }
            }
        });
        info.ids = $.unique(info.ids).sort().join("\n");
        info.dataids = $.unique(info.dataids).sort().join("\n");
        info.classes = $.unique(info.classes).sort().join("\n");
        console.log(info.ids);
        console.log(info.dataids);
        ...
        if ($(".mainsidebar-wrapper").length) {
            ...
            var getheadermenuposition = function(){
                if(issidenav) {
                    ...
                    if($(".section-footer")[0]) bottommargin = $(".section-footer").height();
                    else if($(".landing-footer")[0]) bottommargin = $(".footercontent").height();
            ...
            var followsidebar = function () {
                ...
                if (ww > 1152 && enabled) {
                    /*if (sidebarmode === 'stickbottom') {*/
                        var offsetNewH = 0, diffFooterH;
                        var currentWindowBottom = window.jpobj.globaltop + wh;
                        sidebarpos = currentWindowBottom - sidebarheight - parentpos;
                        offsetNewH = -(sidebarheight - wh);
                        if($(".section-footer")[0] && window.jpobj.globaltop + wh>$(".section-footer").offset().top) {
                            diffFooterH = currentWindowBottom-$(".section-footer").offset().top;
                            offsetNewH -= diffFooterH;
                        }

                        if(sidebarpos > 0) {
                            if($(sidebar).css('top')!=offsetNewH+'px') {
                                if(!diffFooterH) {
                                    $(sidebar).css({
                                        'top': offsetNewH,
                                        'position': 'fixed'
