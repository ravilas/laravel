...
(function ($) {
    ...
    $.fn.jportfolio = function (options) {
        ...
        if (joption.ismobile) {
            options.loadAnimation = 'fade';

            $(".portfolioitem .info").each(function() {
                $(this).addClass('infoMobile');
            });
        }
        if($(window).width() > 480) {
            $(".filterfloatlist ul").each(function() {
                $(this).addClass('landingmasonryclear');
            });
            $(".filterfloatlist ul li").each(function() {
                $(this).addClass('btn').addClass('btn-sm');
            });
        }
            ...
            var resize_portfolio_item_list = function () {
                ...
                $(".portfolioitem", element).each(function (i) {
                    ...
                    if (options.tiletype === 'normal' && ww > 1024) {
                        ...
                        if($(".titlePortfolio", this)[0]) res.itemheight += parseFloat($(".titlePortfolio", this).height());
                    ...
                    $(".info", this).css({
                        "margin-top": !joption.ismobile?( itemheight - infoheight ) / 2:0
                    });
        ...
        var item_animation = function () {
            var doanimation = function () {
                ...
                var allElementsLength = $(".portfolioitem").length;
                $('.portfolioitem').each(function (index) {
                    if(index == 0 || index == allElementsLength-1) {
                        var element = $(this);
                        var thisoffset = ( $(element).parent().data('offset') == undefined ) ? defaultoffset : $(element).parent().data('offset');

                        var functionAnimateSequence = function (direction) {
                            $(container).imagesLoaded({ background: true }, function() {
                                if($(container).hasClass(animationEnded)) {
                                    $(container).removeClass(animationEnded);
                                    $(loader).fadeOut("slow");
                                    $.animate_load(options.loadAnimation, container, $(container).find('.portfolioitem'), function () {
                                        $(container).isotope('layout');
                                        $(container).addClass(animationEnded);
                                    });
                                }
                            });
                        };

                        element.waypoint(functionAnimateSequence, {
                            offset: thisoffset + '%',
                            triggerOnce: false,
            ...
            if(!$('.page-id-731')[0] && !joption.ismobile) {
                $(window).bind('load', doanimation);
            }
            ...
            var filterclicked = function (event) {
                ...
                $('.btn_portfolio_clicked').each(function() {
                    $(this).removeClass('btn_portfolio_clicked');
                });
                $(this).addClass('btn_portfolio_clicked');
                ...
                var portfoliofilterwidth = $(portfoliofilter).width() - 80;
                ...
                $.animate_hide('fade', container, $(container).find('.portfolioitem'), function () {
            ...
            var portfolioFiltered = 0, allCategoriesBtn;
            $(".filterfloatlist li").each(function(){
                if($(this).text().toLowerCase() == 'all categories') {
                    allCategoriesBtn = this;
                }

                if($(this).text().toLowerCase() == $("[name='category']", portfolioform).val().toLowerCase() 
                    && $("[name='page']", portfolioform).val() == 1) {
                    $(this).click();
                    portfolioFiltered = 1;
                }
            });
            if(!portfolioFiltered && joption.ismobile) {
            ...
            // step by step
            if(!joption.ismobile) item_animation();
            filterlistclicked();
        });
    };
    var sendHeight = function() {
        window.parent.postMessage({"height": $('.contentholder').height()}, "*")
    }
